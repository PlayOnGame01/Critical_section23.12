#include "Critickal_sectionDlg.h"
...
DWORD WINAPI WriteToFiles(LPVOID lp)
{
	...
	CopyFile(L"C:\\Пользователь\\ADMIN\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy.txt",
		L"C:\\Пользователь\\ADMIN\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy2.txt", 0);
	if (create2.is_open()) {
		MessageBox(0, TEXT("Копия 2 была создана"), TEXT(""), MB_OK);
	}
	create2.close();
	ofstream create3(TEXT("Copy3.txt"));
	CopyFile(L"C:\\Пользователь\\ADMIN\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy.txt",
		L"C:\\Пользователь\\ADMIN\\Downloads\\Критические секции\\CriticalSection\\CriticalSection\\Copy3.txt", 0);
	if (create3.is_open()) {
		MessageBox(0, TEXT("Копия 3 была создана"), TEXT(""), MB_OK);
	}
	create3.close();
...
DWORD WINAPI ReadFromFiles(LPVOID lp)
{
	...
	if (!in2.is_open()) {
		MessageBox(0, TEXT("Fatal Error"), TEXT("Fatal Error"), MB_OK);
		return 1;
	}
	in2.close();
